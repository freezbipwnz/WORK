...
function effectChips(item: CatalogItem): string[] {
  ...
  if (item.facadeBonus) chips.push(`🚶 +${item.facadeBonus}% гостей`)
  return chips
}
    ...
    >
      ...
      <div className="flex flex-wrap gap-1">
        {effectChips(item).map((c) => (
          <span
            key={c}
            className="rounded-md bg-cream px-1 py-px text-[0.625rem] font-semibold text-cocoa-soft"
          >
            {c}
          </span>
        ))}
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5">
        {CATEGORIES.map((c) => (
          <motion.button
            ...
            onClick={() => setCat(c.id)}
            className={cn(
              'flex min-h-[44px] shrink-0 items-center gap-1 rounded-xl px-3 text-[0.8125rem] font-bold shadow-sticker outline-cozy',
              cat === c.id
