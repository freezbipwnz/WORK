...
function SceneViewport() {
  ...
  const onPointerMove = (e: ReactPointerEvent) => {
    ...
    if (ptrs.current.size === 2) {
      ...
      if (!g.moved && Math.hypot(p.x - g.downX, p.y - g.downY) > TAP_SLOP) g.moved = true
  ...
  return (
    ...
    >
      ...
      {/* кнопки зума (тач-таргеты 44px) */}
      <div className="absolute bottom-2 right-2 z-[60] flex flex-col gap-2">
        <button
          type="button"
          aria-label="Приблизить"
          className="outline-cozy flex h-11 w-11 items-center justify-center rounded-full bg-paper font-display text-xl font-bold text-cocoa shadow-panel active:scale-95"
